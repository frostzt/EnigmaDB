...
namespace utility {
    ...
    /**
     * Encodes a 64-bit unsigned integer into a variable-length integer format
     * and appends it to the given output vector.
     *
     * The variable-length format encodes integers in a compact manner by using
     * the least significant 7 bits of each byte to store the integer's value,
     ...
     * @param out The output vector to which the encoded variable-length integer
     *            will be appended.
     * @param value The 64-bit unsigned integer value to be encoded.
     */
    ...
    /**
     * Decodes a variable-length 64-bit unsigned integer from the provided data
     * pointer and updates the output parameter with the decoded value.
     *
     * The variable-length integer is encoded using the least significant 7 bits
     * per byte, with the most significant bit (MSB) serving as a continuation flag.
     * Decoding continues until the continuation flag (MSB) is cleared or an error
     * occurs due to invalid input or bounds exceeded.
     *
     * @param status A reference to a StatusCode variable where the status of
     *               the operation will be set. It will be set to `StatusCode::OK`
     *               if decoding is successful, `StatusCode::TRUNCATED` if the input
     *               data runs out before decoding completes, or `StatusCode::MALFORMED`
     *               if the input exceeds the maximum expected size of 10 bytes.
     * @param data A pointer to the beginning of the input data to be decoded.
     * @param limit A pointer to the end of the input data range. Used to ensure
     *              decoding does not exceed the available input bounds.
     * @param out A reference to a 64-bit unsigned integer where the decoded value
     *            will be stored if decoding is successful.
     *
     * @return A pointer to the next byte in the input data after decoding is
     *         complete, or `nullptr` if an error occurred during decoding.
     */
    inline const uint8_t *getVarint64(StatusCode &status, const uint8_t *data, const uint8_t *limit, uint64_t &out) {
        status = StatusCode::OK;
        uint64_t result = 0;
        int shift = 0;

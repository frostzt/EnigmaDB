...
private:
    ...
    static void insert(std::unique_ptr<Node<T> > &root, const T &key);

    static void remove(std::unique_ptr<Node<T> > &root, const T &key);
    ...
    static void printTree(const std::unique_ptr<Node<T> > &node, int indent);

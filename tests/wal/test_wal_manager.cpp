//
// Created by frostzt on 7/26/2025.
//
...
TEST_CASE("should handle concurrent appends with rotation", "[WAL]") {
    std::string path = "wal";
        ...
        threads.emplace_back([&manager, &threadResults, i]() {
            for (size_t j = 0; j < perThreadCount; ++j) {
                const bool ok = manager.append(
                    core::Entry("customer", core::Key{{TESTS::makeField("cid_" + std::to_string(j))}}, {}, false));
